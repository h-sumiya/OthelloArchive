<script>
    ...
    import BitBoard from "./sample/bitBoard.svelte";
    let select = null;
</script>
...
<div class="body">
    ...
    <div class="inner">
        ...
        {#if select === 0}
            ...
            <p>
                オセロAIでは大量の計算が必要となるためRustで開発を行いました。
            </p>
            <p>
                公式チュートリアルはこちら<a
        ...
        {:else if select === 2}
            ...
            <p>
                今回のプロジェクトでは短い期間で開発するためにSvelteを使用しました。
            </p>
            <p>
                公式チュートリアルはこちら<a
    ...
    <div class="inner">
        ...
        <p>
            このような方法を用いる理由は一度にすべての着手可能なマスを求めたり、着手後の盤面を求める処理を非常に高速に行うことが可能になるためです。
            (複雑な操作になるためここでは説明を省略)
            更に2つの数字で1ターン分のデータを表すことができるためメモリの使用量も少なく済みます。
        </p>
        <BitBoard />
        <div class="info">
            操作の例:0x7f7f7f7f7f7f7f(盤面の上、左のみビットが立っていない64bit整数)と黒盤の9bit右にシフトしたものの論理積をとり左上に盤面を移動させている。
        </div>
        <p>
